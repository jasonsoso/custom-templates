$(function() {
    //屏蔽输入script弹层
    $(document).on("keyup", ".input,.diy-videowebsite input", function() {
        var vals = $(this).val();
    ...
    HYD.DIY.Unit.event_type1 = function(ctrldom, data) {
        ...
        //模块上下边距：
        $ctrl.find(".j-slider").slider({
            min: 0,
            max: 50,
            step: 1,
            animate: "fast",
            value: data.content.modulePadding,
            slide: function(event, ui) {
                $conitem.find(".modulePadding").css({
            ...
            stop: function(event, ui) {
                data.content.modulePadding = parseInt(ui.value);
            }
        });
    };
    ...
    HYD.DIY.Unit.event_type2 = function(ctrldom, data) {
        ...
        var reRender = function() {
            ...
            HYD.DIY.Unit.event_type2($ctrl, data);
        }

        //主标题
        $ctrl.find("input[name='title'],input[name='direction'],input[name='style']").change(function() {
            var val = $(this).val(),
                typename=$(this).attr("name");

            data.content[typename] = val;
    ...
    HYD.DIY.Unit.event_type3 = function(ctrldom, data) {
        ...
        var add = function() {
            HYD.popbox.ModulePicker(function(module) {
                $conitem.find(".type3_custModule").text(module.title); //修改手机内容
                //去掉添加按钮，替换为修改按钮
                var html = _.template($("#tpl_diy_ctrl_type3_modify").html(), {
                        content: module
                    }),
                    $render = $(html);
    ...
    HYD.DIY.Unit.event_type4 = function(ctrldom, data) {
        ...
        var setImgHeightcallback = function() {
            $conitem.find('.mingoods,.biggoods,.b_mingoods').each(function(index, el) {
                var me = $(this),
        ...
        //改变内容效果
        $ctrl.find("input[name='goodstyle']").change(function() {
            var val = $(this).val();
            data.content.goodstyle = val; //同步数据到缓存
            reRender(setImgHeightcallback);
        });
        ...
        //按钮文本修改
        $ctrl.find("input[name='goodstxt']").change(function() {
            var val = $(this).val();
            $conitem.find(".goods-btn a").text($(this).val());
            data.content.goodstxt = val;
            reRender(setImgHeightcallback);
        });

        //标题是否多行显示
        $ctrl.find("input[name='multiLine']").change(function() {
            if($(this).attr("checked")){  
                $(this).attr("checked",'true');
                data.content.titleLine = 1; //同步数据到缓存     
            }else{
                $(this).removeAttr("checked"); 
                data.content.titleLine = 0; //同步数据到缓存 
            }
            reRender(setImgHeightcallback);
        });
        ...
        // 图片左移
        $ctrl.find('.img-list>li .img-move-left').on('click', function() {
            var me = $(this);
            ...
            if (curindex != 0) {
                ...
                //替换缓存数组中的位置
                var tmpdata = data.content.goodslist.slice(curindex + 1, curindex + 2)[0];
                data.content.goodslist.splice(curindex + 1, 1);
                data.content.goodslist.splice(curindex, 0, tmpdata);
                reRender(setImgHeightcallback);
            };
            return false;
        });
        ...
        // 图片右移
        $ctrl.find('.img-list>li .img-move-right').on('click', function() {
            var me = $(this),
            ...
            if (curindex != (len - 1)) {
                ...
                //替换缓存数组中的位置
                var tmpdata = data.content.goodslist.slice(curindex, curindex + 1)[0];
                data.content.goodslist.splice(curindex, 1);
                data.content.goodslist.splice(curindex - 1, 0, tmpdata);
                reRender(setImgHeightcallback);
            };
            return false;
        });
    ...
    //模块类型6 商品搜索
    HYD.DIY.Unit.event_type6 = function(ctrldom, data) {
        var $conitem = data.dom_conitem, //手机内容
            $ctrl = ctrldom, //控制内容
    ...
    HYD.DIY.Unit.event_type8 = function(ctrldom, data) {
        ...
        //改变布局\样式
        $ctrl.find("input[name='layout'],input[name='layout1_style']").change(function() {
            var name = $(this).attr("name");

            data.content[name] = $(this).val();
        ...
        $ctrl.find(".droplist li").click(function() {
            ...
            HYD.popbox.dplPickerColletion({
                ...
                callback: function(item, type) {
                    ...
                    data.content.dataset[index].link = item.link;
                    data.content.dataset[index].linkType = type;
                    // data.content.dataset[index].is_compress=item.is_compress;
                    if (item.pic && item.pic != "") {
                        if (item.is_compress) {
                            data.content.dataset[index].pic = item.pic + '480x480';
                        } else {
        ...
        //改变文字背景颜色
        $ctrl.find(".j-imgNav-cp").each(function(index,item){
            var initColor=$(this).data("#FE9303");
            ...
            $(this).ColorPicker({
                ...
                onChange: function (hsb, hex, rgb) {
                    var hex='#' + hex;
                    data.content.dataset[index].titleBackgroundColor=hex;
                    reRender();
                }
            });
        });
    ...
    HYD.DIY.Unit.event_type13 = function(ctrldom, data) {
        ...
        $ctrl.find('input[name="layout"]').change(function(event) {
            ...
            //新版本中不需要进行计算高度了
            if (data.content.version && data.content.version == 2) {
                reRender();
            } else {
        ...
        $ctrl.find(".j-selectimg").click(function() {
            ...
            HYD.popbox.ImgPicker(function(imgs) {
                if(imgs[0].indexOf("@")<0){
                    data.content.dataset[index].pic = imgs[0] + "@"; //获取第一张图片
                    data.content.dataset[index].is_compress = 1;
                }
                else{
                    data.content.dataset[index].pic = imgs[0];
                    data.content.dataset[index].is_compress = 0;
                }
                //新版本中不需要进行计算高度了
                if (data.content.version && data.content.version == 2) {
                    reRender();
        ...
        //是否显示文字
        $ctrl.find(".j-showTitle").change(function() {
            var index = $(this).parents("li.ctrl-item-list-li").index();
            data.content.dataset[index].showTitle = $(this).val();
            reRender();
        });
            ...
            var showImgListRender = function(page, callback_showImgListRender) {
                var dorender = function(data) {
                    ...
                    if (!imglist || !imglist.length) {
                        ...
                        //渲染模板
                        var html = _.template($("#tpl_popbox_ImgPicker_audio").html(), {
                                dataset: imglist
                            }),
                            $render = $(html);
    ...
    HYD.DIY.Unit.event_type16 = function(ctrldom, data) {
        ...
        $ctrl.find("select[name='navbgColor']").change(function() {
            var val = $(this).val();
            data.content.bgColor = val;
            reRender();
        });

        //颜色选择器
        $ctrl.find(".colorPicker").each(function(indexs, el) {
            var _this = $(this);
            if (indexs == 0) {
                var name = $(this).data("name"),
                    color = $(this).data("color"),
        ...
        //选择字体大小
        $ctrl.find("input[name='fontSize']").change(function() {
            var val = $(this).val();
            $conitem.find(".notice-con").removeClass("font12 font14 font16").addClass(val);
            data.content.fontSize = val;
        });
